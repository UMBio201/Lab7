...
knitr::opts_knit$set(root.dir = "~/Box Sync/Schmidt_Lab/Bio201_teaching/Lab7")
```
...
```{r}
plot_grid(brm2_plot, brm1_plot_new, leg_1day, nrow = 1, ncol = 3)
```
...
physq_fam <- physq_obj %>% 
  ...
  # Calculate weekly mean of relative abundance
  group_by(participant_id, study_week, semester, 
           frequency, supplement_consumed, Family, OTU) %>%
  summarise(relative_abundance = mean(Abundance, na.rm = TRUE)) %>%
  # remove low abundance taxa
...
```{r}
# calculate richness as above
richness_df2 <- physq_obj %>%
  subset_samples(supplement_consumed != "LOODAT") %>% # filter phyloseq obj to remove LOODAT 
  estimate_richness(., split = TRUE,  measures = c("Observed")) %>% 
  rownames_to_column(var = "sample_id") %>%
  inner_join(samp_data, by = "sample_id") %>%  
  rename(richness = Observed) %>%
...
```{r}
# susbet for one twice a day 
rich_2x <- richness_df2 %>%
...
rich_wk3 <- richness_df %>%
  filter(study_week == "week3")
shapiro.test(rich_wk1$avg_richness) #p-value = 0.8874
shapiro.test(rich_wk3$avg_richness) # p-value = 0.6605
# histogram and qqplot for normality checks
ggplot(rich_wk1, aes(x=avg_richness)) +
  geom_histogram() 
...
# format data for test
rich_df_wide <- rich_2x %>%
  spread(key = "study_week", value = "avg_richness") %>%
  drop_na()

# variance
var.test(rich_df_wide$week1, rich_df_wide$week3, 
            alternative = "two.sided", paired = TRUE) # p-value = 0.1244

# conduct test 
t.test(rich_df_wide$week1, rich_df_wide$week3, 
            alternative = "two.sided", paired = TRUE, var.equal = FALSE) # p-value = 3.669e-07
```
...
```{r}
# susbet for one once a day 
rich_1x <- richness_df2 %>%
...
rich_wk3 <- richness_df %>%
  filter(study_week == "week3")
shapiro.test(rich_wk1$avg_richness) #p-value = 0.6372
shapiro.test(rich_wk3$avg_richness) # p-value = 0.6605
# histogram and qqplot for normality checks
ggplot(rich_wk1, aes(x=avg_richness)) +
  geom_histogram() 
...
# format data for test
rich_df_wide <- rich_1x %>%
  spread(key = "study_week", value = "avg_richness") %>%
  drop_na()

# variance
var.test(rich_df_wide$week1, rich_df_wide$week3, 
            alternative = "two.sided", paired = TRUE) # p-value = 0.06657

# conduct test 
t.test(rich_df_wide$week1, rich_df_wide$week3, 
            alternative = "two.sided", paired = TRUE, var.equal = FALSE) # p-value = 0.1633
```
...
physq_j <- ordinate(physq_scale, 
           method = "NMDS", 
           k=3, maxit=500, try=50,
           distance = "jaccard") #stress = 0.1883008 
physq_j
```

```{r}
# plot
plot_ordination(physeq = physq_sub, 
                     ordination = physq_j, 
                     type = "samples", 
                     color = "study_week", 
                     shape = "study_week") +
...
```{r}
# statistical test 
# calculate jaccard index, get distance matrix
...
# run test
adn_resj <- adonis(formula = dat_jac ~ study_week, 
                  data = sampledf)
